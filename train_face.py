...
my_faces_path = './GruuyFace'
other_faces_path = './otherFace'
...
def readData(path,h=size,w=size):
    for filename in os.listdir(path):
        if filename.endswith('.jpg'):
            ...
            #将图片放大，扩充图片边缘部分，方便变成64x64  resize等比缩放
...
#将图片数组与标签转换成数组，并给图片做上标签(one-hot)
imgs = np.array(imgs)
labs = np.array([[0,1] if lab == my_faces_path else [1,0] for lab in labs])
 
#随机划分测试集和训练集，规定测试集的大小，这里是可以自己调的     test_size测试集占比  random_state随机标记 避免重复随机数
train_x,test_x,train_y,test_y = train_test_split(imgs,labs,test_size=0.05,random_state=random.randint(0,1000))
...
#filters 卷积核个数  kernel_size 卷积核大小 strides 步长 padding 边界填充方式 activation 指定激活函数
def cnnlayer():
        ...
        for n in range(10):
            ...
            for i in range(num_batch):
                ...
                if (n * num_batch + i) % 100 == 0:
                    ...
                    # 准确率大于0.98时保存并退出
                    if acc > 0.98 and n > 2:
                        saver.save(sess, './Gruuy.model', global_step=n * num_batch + i)
                        sys.exit(0)
